...
int main (int argc, char* argv[])
{
    ...
    //This is the line to uncomment for Arthas
    const size_t initialSize = 10;
    //const size_t initialSize = 1024*16*4;
    // const size_t initialSize = 1000;

    TOID(CCEH) HashTable = OID_NULL;
    PMEMobjpool* pop;
    if(access(path, 0) != 0){
	//pop = create_pmem(path, "CCEH", POOL_SIZE*20, 0666);
	pop = create_pmem(path, "CCEH", POOL_SIZE, 0666);
